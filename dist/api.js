...
function parseNumstat(text) {
  ...
  for (const line of text.split("\n")) {
    ...
    const path2 = rawPath.startsWith('"') ? parsePathToken(rawPath, 0)?.value ?? rawPath : rawPath;
    const isBinary = addRaw === "-" && delRaw === "-";
    const additions = isBinary ? 0 : parseInt(addRaw, 10);
    const deletions = isBinary ? 0 : parseInt(delRaw, 10);
    const openBrace = path2.indexOf("{");
    const closeBrace = path2.lastIndexOf("}");
    if (openBrace !== -1 && closeBrace > openBrace) {
      const inner = path2.slice(openBrace + 1, closeBrace);
      const arrow2 = inner.indexOf(" => ");
      if (arrow2 !== -1) {
        const oldInner = inner.slice(0, arrow2);
        const newInner = inner.slice(arrow2 + 4);
        const prefix = path2.slice(0, openBrace);
        const suffix = path2.slice(closeBrace + 1);
    ...
    if (arrow !== -1) {
      out.set(path2.slice(arrow + 4), {
        additions,
        deletions,
        isBinary,
        from: path2.slice(0, arrow)
      });
    } else {
      out.set(path2, { additions, deletions, isBinary });
    }
  }
  return out;
}
function buildDiffMap(baseSha, headSha, diffText, numstatText, nameStatusText) {
  ...
  for (const s of sections) {
    const path2 = s.status === "deleted" ? s.oldPath : s.newPath;
    if (path2 === "") continue;
    const ns = numstat.get(path2) ?? { additions: 0, deletions: 0, isBinary: false };
    ...
    files.set(path2, {
      path: path2,
      status: s.status,
      previousPath: s.previousPath && s.previousPath !== path2 ? s.previousPath : void 0,
      additions: ns.additions,
      deletions: ns.deletions,
      isBinary: s.isBinary || ns.isBinary,
      lines: s.status === "deleted" ? 0 : maxNew
    });
    hunks.set(path2, s.hunks);
    addedLines.set(path2, added);
  }
  return { baseSha, headSha, files, hunks, addedLines };
}
function renderFileDiff(diff, path2) {
  const file = diff.files.get(path2);
  if (!file) return null;
  const hunks = diff.hunks.get(path2) ?? [];
  const parts = [];
  parts.push(`diff --git a/${file.previousPath ?? path2} b/${path2}`);
...
function classifyPath(path2) {
  if (path2 === ".") return "root";
  if (path2 === "") return "root";
  const n = normalizeRepoPath(path2);
  if (n === null) {
    if (path2.startsWith("/") || /(^|\/)\.\.([/]|$)/.test(path2)) return "escape";
    return "invalid";
  }
  return "ok";
}
...
function classifyFile(path2, isBinary, config) {
  if (isBinary) {
    return { eligible: false, kind: "binary", reason: "excluded:binary-file" };
  }
  if (config.exclude.length > 0 && matchesAny(path2, config.exclude)) {
    return { eligible: false, kind: "config-exclude", reason: "excluded:config-exclude" };
  }
  const includeMatches = config.include.length > 0 ? matchesAny(path2, config.include) : null;
  ...
  if (includeMatches === null) {
    const rule = DEFAULT_EXCLUDE_RULES.find((r) => minimatch(path2, r.pattern, { dot: true }));
    if (rule) {
      return {
        eligible: false,
        kind: rule.kind,
...
var GitHubClient = class {
  ...
  async api(method, path2, body) {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      signal.throwIfAborted();
      try {
        const response = await this.fetchImpl(this.base + path2, {
          method,
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new GitHubError(
            rateLimited ? "GitHub rate limit exhausted" : "GitHub API HTTP " + response.status + " for " + method + " " + path2,
            response.status,
        ...
        try {
          ...
          throw new GitHubError("GitHub API returned non-JSON for " + method + " " + path2, void 0, true);
        }
      } catch (err) {
  ...
  async paginate(path2) {
    const out = [];
    for (let page = 1; page <= 100; page++) {
      const items = await this.api("GET", path2 + "?per_page=100&page=" + page);
...
function checkpoint(ctx) {
  const facts = {
    ...
    diff_pages: [...ctx.diffReadPages ?? []].map(([path2, lines]) => ({ path: path2, delivered_lines: lines.size })),
...
var postInlineReviewCommentTool = {
  ...
  async execute(args, ctx) {
    ...
    const path2 = requireString(a, "path");
    if (!path2) return { ok: false, result: null, error: 'missing required argument "path"' };
    ...
    const norm2 = normalizeRepoPath(path2);
    if (!norm2) {
      return reject(`invalid or escaping path "${path2}"`);
    }
...
var completeReviewFileTool = {
  ...
  async execute(args, ctx) {
    const path2 = requireString(args, "path");
    const summary = requireString(args, "summary");
    if (!path2 || !ctx.batchFiles.includes(path2)) return { ok: false, result: null, error: "path must be a file assigned to this batch" };
    ...
    if (!ctx.diffReads.has(path2)) return { ok: false, result: null, error: "read all diff pages without truncation before completing this file" };
    ctx.fileCompletions ??= /* @__PURE__ */ new Map();
    if (!ctx.fileCompletions.has(path2)) {
      ctx.fileCompletions.set(path2, { summary, batchIndex: ctx.postState.batchIndex });
      ctx.onProgress?.({ type: "file-completed", file: path2, batchIndex: ctx.postState.batchIndex, elapsedMs: ctx.budget.elapsed() });
    }
    return { ok: true, result: { path: path2, completed: true } };
  }
};
